...
function WebsiteHeader({ url }: { url: string }) {
  return (
    <div className="flex items-center justify-between border-b border-neutral-100 px-2 pt-2 pb-1 transition-colors duration-300 dark:border-neutral-700">
      ...
      <div className="flex h-4 w-fit items-center rounded-sm bg-neutral-200 px-2 py-0.5 dark:bg-neutral-700">
    ...
    <div className="relative mt-6 h-40 w-full max-w-80 sm:mt-10 sm:max-w-lg">
      {cards.map((card, index) => {
        return (
          <motion.div
            key={card.id}
            className="absolute flex h-40 w-full max-w-80 flex-col justify-between rounded-sm border border-neutral-200 bg-white transition-colors duration-300 sm:max-w-lg dark:border-neutral-700 dark:bg-neutral-900"
            style={{
